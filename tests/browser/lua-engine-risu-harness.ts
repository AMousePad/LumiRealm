...
try {
  ...
  for (const [index, fixture] of engineCases.entries()) {
    ...
    for (const kind of ['risu', 'wasmoon', 'fengari'] as const) {
      ...
      const differs = kind === 'fengari' ? fixture.fengariDiverges : kind === 'wasmoon' ? fixture.wasmoonDiverges : false;
      try {
        if (differs && process.env.RISU_PARITY_STRICT !== '1') assert.notDeepEqual(row[kind], expected, `${kind}: ${fixture.name} now matches Risu; remove the expected-failure marker`);
        else assert.deepEqual(row[kind], expected, `${kind}: ${fixture.name}`);
      } catch (error) {
        failures.push(String(error));
      }
    }
  }
  const report = { oracle: risuSource, browser: browser.version(), cases: results };
