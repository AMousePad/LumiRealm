...
export const cbsRisuCases: readonly CbsRisuCase[] = [
  ...
  { name: 'dictionary splits at the first equals sign', template: '{{dict::a=A=B}}', expected: '{"a":"A=B"}', source: 'cbs.ts:1305 registerCBS(makedict)', knownFailure: true },
  { name: 'dictionary ignores arguments without equals signs', template: '{{dict::ignored::a=A::a=B}}', expected: '{"a":"B"}', source: 'cbs.ts:1305 registerCBS(makedict)', knownFailure: true },
  ...
  { name: 'last character alias searches before the current message', template: '{{lastcharmessage}}', expected: 'Greeting', source: 'cbs.ts:195 registerCBS(previouscharchat)', mode: 'display', knownFailure: true },
  { name: 'last user alias is empty without a message index', template: '{{lastusermessage}}', expected: '', source: 'cbs.ts:214 registerCBS(previoususerchat)', knownFailure: true },
  ...
  { name: 'history greeting has no invented timestamp', template: '{{history}}', expected: JSON.stringify([JSON.stringify({ role: 'char', data: 'Greeting' }), JSON.stringify({ role: 'user', data: 'Hello', time: 1700000000000 }), JSON.stringify({ role: 'char', data: 'Welcome', time: 1700000005000 })]), source: 'cbs.ts:1513 registerCBS(history)', knownFailure: true },
  { name: 'counted cbr preserves the registered callback behavior', template: '{{cbr::2}}', expected: 'cbr::2cbr::2', source: 'cbs.ts:1386 registerCBS(cbr)', knownFailure: true },
  ...
  { name: 'missing startswith arguments preserve the original macro', template: '{{startswith}}', expected: '{{startswith}}', source: 'cbs.ts:985 registerCBS(startswith); parser/parser.svelte.ts:1071 matcher', knownFailure: true },
  { name: 'inactive setvar preserves the exact supplied syntax', template: '{{setvar}}', expected: '{{setvar}}', source: 'cbs.ts:825 registerCBS(setvar); parser/parser.svelte.ts:1805 risuChatParser', knownFailure: true },
