...
export const cbsRisuCases: readonly CbsRisuCase[] = [
  ...
  { name: 'function empty return remains empty', template: '{{#func f}}x{{return::}}y{{/func}}{{call::f}}', expected: '', source: 'cbs.ts:779 registerCBS(return); parser/parser.svelte.ts:1816 risuChatParser' },
  ...
  { name: 'unknown ignore block retains delimiters and parses its body', template: '{{#ignore}}A {{char}} B{{/ignore}}', expected: '{{#ignore}}A Character B{{/ignore}}', source: 'parser/parser.svelte.ts:1188 blockStartMatcher; :1805 risuChatParser' },
  { name: 'top-level declare stays literal without a caller variable map', template: '{{declare::x}}', expected: '{{declare::x}}', source: 'cbs.ts:2249 registerCBS(declare); parser/parser.svelte.ts:1071 matcher' },
  { name: 'missing calc argument preserves the original macro', template: '{{calc}}', expected: '{{calc}}', source: 'cbs.ts:802 registerCBS(calc); parser/parser.svelte.ts:1071 matcher' },
